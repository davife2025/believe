    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-4 pt-5 pb-4 border-b border-[var(--border-subtle)]">
        <Link href="/" className="flex items-center gap-3 mb-4 group" onClick={onNavClick}>
          ...
          <div>
            ...
            <p className="text-[10.5px] text-[var(--text-disabled)] mt-0.5 leading-none">Open Learning Platform</p>
          </div>
        </Link>
      ...
      <nav className="flex-1 px-3 py-3 overflow-y-auto space-y-0.5">
        ...
        {MAIN_NAV.map(({ href, label, Icon, color }) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <Circle size={5} fill={color} color={color} className="flex-shrink-0 opacity-70" />
              )}
            </Link>
          )
        })}
        ...
        <div className="pt-3">
          ...
          {PERSONAL_NAV.map(({ href, label, Icon }) => {
      ...
      <div className="px-4 py-4 border-t border-[var(--border-subtle)]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[12.5px] font-medium text-[var(--text-secondary)] truncate">You</p>
            <p className="text-[10.5px] text-[var(--text-disabled)]">Personal workspace</p>
          </div>
...
function MobileHeader({ onOpen }: { onOpen: () => void }) {
  ...
  return (
    <header className="lg:hidden fixed top-0 left-0 right-0 z-40 h-14 flex items-center gap-3 px-4 border-b border-[var(--border-subtle)]"
      ...
      <div className="flex items-center gap-2 flex-1 min-w-0">
        ...
        <p className="text-[13.5px] font-semibold text-[var(--text-secondary)] truncate">
          {current?.label ?? 'Believe'}
        </p>
      </div>

      <Link href="/personal/bookmarks"
        className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--text-tertiary)] hover:text-[var(--text-secondary)] hover:bg-white/6 transition-all">
        <Bookmark size={16} />
      </Link>
    </header>
